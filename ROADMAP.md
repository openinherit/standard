...
*Schema changes merged to `main`. npm publish pending ([Richard Task #41](https://github.com/openinherit/standard/issues)).*
...
Built with Hugo, deployed on GitHub Pages. Extension data flows from the standard repo to the www site (currently vendored; will switch to Hugo Modules when the repo goes public).
...
- CherishedItems brand retired — four TT brands remain: LegacyLists, MyFamilyInherits, WillsInFaith, WillScan.ai
...
Rename `openinherit/standard` to `openinherit/standard` and make it public. This enables Hugo Module imports for the www site, Go module proxy caching, and community contributions via pull requests. Migration guide: [fresh-to-standard-migration.md](docs/implement/fresh-to-standard-migration.md).
