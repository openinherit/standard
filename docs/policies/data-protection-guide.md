...
|--------|--------|--------------------------------|
| **Person** | givenName, familyName, additionalName, preferredName, dateOfBirth, contact (email, phone, address) | Legitimate interest (estate administration) or contract (service subscription) |
...
When estate data is used for item disposal through a marketplace or selling platform or external channels:
...
4. **Data controller boundaries.** The estate-planning application and any marketplace or selling platform should have clear data boundaries with separate privacy policies. Data flows between them only via explicit user action (INHERIT JSON export/import).
