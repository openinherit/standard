...
A pre-commit and CI gate for this public repository. It refuses internal-only
files matched by PATH (agent-instruction files and internal working
directories) and flags local-machine / operator paths found in file contents.
A product or brand NAME is never a hit — a name in a schema example is the
...
# --- Category 4: local-machine / operator paths in file contents. -----------
_TERM_RULES = [
    (4, re.compile(r"/mnt/c/Users", re.I), "local-machine path"),
    (4, re.compile(r"[A-Za-z]:\\Users\\", re.I), "local-machine path"),
]

# `/home/<name>/` is a hit EXCEPT well-known CI account names.
...
def main(argv):
    ...
    if not findings:
        ...
        print(
            ...
            "matched — it does not mean no non-public content exists."
        )
        return 0
