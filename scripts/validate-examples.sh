...
# A catalogue document is not an estate document: it validates against
# v3/catalogue.json, not v3/schema.json. Routed to the catalogue arm below
# rather than skipped — being on the SKIP list is what hid the room gap.
CATALOGUE_SCHEMA="v3/catalogue.json"
CATALOGUE_FIXTURES="examples/fixtures/catalogue-only.json packages/conformance/catalogue/valid/catalogue-only.json"
...
for fixture in examples/fixtures/*; do
  ...
  if echo "$CATALOGUE_FIXTURES" | grep -qw "$fixture"; then
    echo "ROUTE: $filename (catalogue — validated against $CATALOGUE_SCHEMA below)"
    continue
  fi
...
# Catalogue documents validate against catalogue.json, not schema.json.
for fixture in $CATALOGUE_FIXTURES; do
  if jsonschema validate "$CATALOGUE_SCHEMA" "$fixture" $RESOLVE_FLAGS 2>/dev/null; then
    echo "PASS: $(basename "$fixture") (catalogue)"
    PASSED=$((PASSED + 1))
  else
    echo "FAIL: $(basename "$fixture") (catalogue)"
    FAILED=$((FAILED + 1))
  fi
done
